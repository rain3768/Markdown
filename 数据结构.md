...
一个无环的有向图称为**有向无环图**，简称**$DAG图$**。

![image-20220605174742856](E:\资料\照片\软件专业的必修课及其关系.png)

![image-20220605174822883](E:\资料\照片\表示课程之间优先关系的有向图.png)

这种用顶点表示活动，用弧表示活动间的优先关系的有向图称为顶点表示活动的**网**，简称**$AOV-网$**。

**拓扑排序**：将$AOV-网$中所有顶点排列成一个线性排序，该序列满足：若在AOV-网中由顶点$v_i$到顶点$v_j$有一条路径，则该线性序列中的顶点$v_i$必定在顶点$v_j$之前。

#### 拓扑排序的过程

![image-20220605175412019](E:\资料\照片\AOV-网及其拓扑有序序列产生的过程.png)
...
{//有向图G采用邻接表存储结构
 //若G无回路，则生成一个拓扑序列topo[]并返回OK，否则ERROR
    FindInDegree(G,indegree);									  //求出各顶点的入度存入数组indegree中
    InitStack(s);												  //栈S初始化为空
    for(i=0;i<G.vexnum;++i)
        if(!indegree[i]) Push(S,i);								  //入读为0者进栈
    m=0;														  //对输出顶点计数，初始为0
    while(!StackEmpty(S))										  //栈S非空
    {
		Pop(S,i);												  //将栈顶顶点vi出栈
        topo[m]=i;												  //将vi保存在拓扑数列topo中
        ++m;													  //对输出顶点计数
...
$AOE-网$：边表示活动的网，顶点表示事件，弧表示活动持续时间。

![image-20220605181033518](E:\资料\照片\AOE-网.png)

**源点**：只有一个入度为0的点

**汇点**：只有一个出度为0的点

**带权路径长度**：一条路径各弧上的权值之和（**路径长度**）

**关键路径**：一条从源点到汇点的带权路径长度最长的路径，上面的活动叫**关键活动**
...
$$
ve(0)=0\\
ve(i)=Max\{ve(k)+w_{k,i}\} \quad <v_k,v_i>\in T,1\le i\le n-1
\tag{6-4}
$$
$T$是所有以$v_i$为头的弧的集合，$w_{k,j}$是弧$<v_k,v_i>$的权值，即对应活动$<v_k,v_i>$的持续时间。

（2）事件$v_i$的最晚发生时间$vl(i)$
$$
vl(n-1)=ve(n-1)\\
ve(i)=Min\{vl(k)-w_{k,i}\} \quad <v_k,v_i>\in S,0\le i\le n-2
\tag{6-5}
$$
$S$是所有以$v_i$为尾的弧的集合。

（3）活动$a_i=<v_j,v_k>$的最早开始时间$e(i)$
$$
e(i)=ve(j)
\tag{6-6}
$$
（4）活动$a_i=<v_j,v_k>$的最晚开始时间$l(j)$
$$
l(i)=vl(k)-w_{j,k}
\tag{6-7}
$$
...
{//G为邻接表存储的有向图，输出G的各项关键活动
    ...
    for(i=0;i<n;i++)
    {
        ...
        while(p!=NULL)
        {
            j=p->adjvex;										   //依次更新k的所有邻接顶点的最早发生事件
    ...
    for(i=n-1;i>=0;i--)
    {
        ...
        while(p!=NULL)											   //根据k的邻接点，更新k的最迟发生时间
        {
            ...
            if(vl[k]>vi-p->weight)								   //更新顶点k的最迟发生时间vl[k]
                vl[k]=vl[j]-p->weight;
            p=p->nextarc;										   //p指向k的下一个邻接顶点
        }														   //while
    }															   //for
    ...
    for(i=0;i<n;i++)											   //每次循环针对vi为活动开始点的所有活动
    {
        ...
        while(p!=NULL)
        {
            ...
            if(e==1)											   //若为关键活动，则输出<vi,vj>
                printf(G>vertices[i].data,G.vertices[j].data);	   //p指向i的下一个邻接顶点
            p=p->bextarc;
        }														   //while
    }															   //for
}
...
{//通过广度优先搜索方法遍历G来验证六度空间理论，Start为指定的一个起点
    ...
    for(len=1;len<=7&&QueueEmpty(Q);len++)						  //统计路径长度不超过7的顶点个数
    {
        ...
        for(w=FirstAdjVex(G,u);w>=0;w=NextAdjVex(G,uw))
            //依次检查u的所有邻接点w，FiestAdjVex(G,u)表示u的第一个邻接点
            //NextAdjvex(G,u,w)表示u相对域w的下一个邻接点，w≥表示存在邻接点
            if(!visited[w])										  //w为u的尚未访问的邻接顶点
            {
...
**顺序查找**的过程：从表的一端开始，依次将记录的关键字和给定值进行比较。
...
{//在顺序表ST中顺序查找其关键字等于key的数据元素。若找到，则函数值为该元素在表中的位置，否则为0
	for(i=ST.length;i>=1;--i)
        if(ST.R[i].key==key) return i;							   //从前往后找
    return 0;
}
```
...
**折半查找（二分查找）**：要求线性表必须采用**顺序存储结构**，而且表中元素按关键字**有序排列**。

**过程**：从表的中间记录开始，如果给定值和中间记录的关键字相等，则查找成功；如果给定值大于或小于中间记录的关键字，则在表中大于或小于中间记录的那一半中查找，这样重复操作，直到成功。或某一步中为区间为空，则失败。

```c
int Search_Bin(SSTable ST,KeyType key)
{//在顺序表ST中折半查找其关键字等于key的数据元素。若找到，则函数值为该元素在表中的位置，否则为0
...
![image-20220607103920408](E:\资料\照片\表及其索引表.png)
...
![image-20220611102451492](E:\资料\照片\直接插入排序过程.png)

```c
void InsertSort(SqList &L)
{//对顺序表L做直接插入排序
    for(i=2;i<=L.length;++i)
        if(L.r[i].key<L.r[i-1].key)								   //“<”，需将r[i]插入到有序子表
        {
